...
> **このドキュメントの対象者**: Windows 環境で開発する方、プロキシ環境で作業する方  
> **使用タイミング**: パス2（標準ハンズオン）の開始前、または開発ツールのインストールが必要な時  
> **目的**: Node.js、Azure CLI、Functions Core Tools などの開発ツールをインストール
...
**macOS / Linux の方へ**: 基本的な手順は同じですが、パッケージマネージャー（Homebrew, apt など）を使用してください。
...
開発環境の準備ができました。次のステップに進みましょう。

### ハンズオンを開始する

- **[🚀 クイックスタート（パス1）](QUICKSTART.md)**: 最短でAPIを動かす
- **[📘 標準ハンズオン（パス2）](README.md)**: Azure へのデプロイと IaC の基礎
- **[🏗️ 本番構成への拡張（パス3）](BICEP_README.md)**: Front Door + APIM + Functions
